...
DATABASE_TYPES = {
    ...
    db.Dremio: {
        ...
        "datetime": [
            "TIMESTAMP",
            # "DATE", # Does not match postgres, which is returning a timestamp.
        ],
        "float": [
            # "FLOAT", # Does not match around the 5th decimal place.
            "DOUBLE",
            "DECIMAL",
            "DEC",
            "NUMERIC",
...
def _insert_to_table(conn, table_path, values, _type):
    ...
    if current_n_rows == N_SAMPLES:
        ...
        _create_table_with_indexes(conn, table_path, _type)
    ...
    elif re.search(r"(time zone|tz)", _type):
    ...
    if isinstance(conn, db.Clickhouse):
        if _type.startswith("DateTime64"):
            ...
            precision = int(_type[8:].rstrip(")").split(",")[1])
            values = [(i, round(sample, precision)) for i, sample in values]
    elif isinstance(conn, db.BigQuery) and _type == "datetime":
